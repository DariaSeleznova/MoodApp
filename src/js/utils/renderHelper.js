...
export function renderFavoriteButton(id, type) {
    return `
        <button class="btn-fav ${isFavorite(id, type) ? 'active' : ''}">
...
export function renderImage(imageUrl, alt, placeholder, extraClass = '') {
    const imageClasses = ['list-card__image', extraClass].filter(Boolean).join(' ');
